    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <?php if (mysqli_num_rows($chatList) > 0): ?>
            <ul class="divide-y divide-gray-100">
                <?php while ($row = mysqli_fetch_assoc($chatList)): ?>
                    <li>
                        <a href="chat_room.php?partner_id=<?= $row['id'] ?>"
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex justify-between items-baseline mb-1">
                                    ...
                                    <span class="text-[11px] text-gray-400 flex-shrink-0 ml-2">
                                        ...
                                        <?php if (date('Y-m-d') != date('Y-m-d', strtotime($row['last_time'])))
                                            echo date(' d/m', strtotime($row['last_time'])); ?>
                                    </span>
                                </div>

                                <div class="flex justify-between items-center">
                                    <p
                                        class="text-sm text-gray-500 truncate pr-4 <?= $row['unread'] > 0 ? 'font-semibold text-gray-800' : '' ?>">
                                        <?= $row['last_message'] ?>
                                    </p>

                                    <?php if ($row['unread'] > 0): ?>
                                        <div
                                            class="flex-shrink-0 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm min-w-[20px] text-center">
        ...
        <?php else: ?>
            <div class="p-10 text-center text-gray-500">
                ...
                <a href="<?= BASE_URL ?>index.php" class="text-[#1E3A8A] font-bold hover:underline mt-2 inline-block">Cari
                    Barang & Chat Penjual</a>
            </div>
        <?php endif; ?>
    </div>
</div>
