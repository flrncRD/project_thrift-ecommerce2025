    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="text-sm text-gray-700">
                        ...
                        <tr class="border-b hover:bg-gray-50 transition">
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="report_transaction.php?id=<?= $row['id'] ?>" title="Laporkan Transaksi"
                            ...
                            <td class="p-4 font-bold text-[#059669]">
                                <?php $nilai_total = isset($row['total_harga']) ? $row['total_harga'] : $row['harga']; ?>
                            ...
                            <td class="p-4">
                                ...
                                <?php else: ?>
                                    ...
                                    ?>
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-bold <?= $colors[$status] ?? 'bg-gray-100 text-gray-600' ?>">
                                        <?= $status ?>
                                    </span>
                                <?php endif; ?>
                            </td>

                            <td class="p-4 text-gray-500"><?= date('d M Y', strtotime($row['createdAt'])) ?></td>

                            <td class="p-4 text-center">
                                <?php if ($view == 'penjualan'): ?>
                                    <?php if ($row['status'] == 'terbayar'): ?>
                                        <a href="../../actions/update_status.php?id=<?= $row['id'] ?>&next=packing"
                                            class="bg-blue-600 text-white px-3 py-1 rounded text-xs hover:bg-blue-700">Packing</a>
                                    <?php elseif ($row['status'] == 'packing'): ?>
                                        <a href="../../actions/update_status.php?id=<?= $row['id'] ?>&next=kirim"
                                            class="bg-purple-600 text-white px-3 py-1 rounded text-xs hover:bg-purple-700">Dikirim</a>
                                    <?php else: ?>
                                ...
                                <?php else: ?>
                                    <?php if ($row['status'] == 'kirim'): ?>
                                        <a href="../../actions/update_status.php?id=<?= $row['id'] ?>&next=terima"
                                            class="bg-green-600 text-white px-3 py-1 rounded text-xs hover:bg-green-700">Terima
                                            Barang</a>
                                    <?php elseif ($row['status'] == 'terima'): ?>
                                        <a href="review.php?id=<?= $row['id'] ?>"
                                            class="bg-yellow-600 text-white px-3 py-1 rounded text-xs hover:bg-yellow-700 mr-2">Review</a>
                                    <?php else: ?>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="p-10 text-center text-gray-400">Belum ada riwayat <?= $view ?>.</td>
                    </tr>
                <?php endif; ?>
