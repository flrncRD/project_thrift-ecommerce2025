        ...
        <div class="md:col-span-2 space-y-6">
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <div class="w-full bg-gray-200 rounded-full h-2.5 mb-2">
                    <?php 
                        $width = '0%';
                        if($data['status']=='Terbayar' || $data['status']=='Packing') $width='33%';
                        if($data['status']=='Kirim') $width='66%';
                        if($data['status']=='Selesai') $width='100%';
                    ?>
                    <div class="bg-[#059669] h-2.5 rounded-full transition-all duration-500" style="width: <?= $width ?>"></div>
                </div>
                <p class="text-xs text-gray-400 text-right">Update terakhir: <?= date('d M Y H:i', strtotime($data['createdAt'])) ?></p>
            </div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <div class="flex gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 text-sm"><?= $data['qty'] ?> x Rp <?= number_format($data['harga'],0,',','.') ?></p>
                        <p class="text-[#059669] font-bold mt-1">Total: Rp <?= number_format($data['total_harga'],0,',','.') ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <?php if($data['jenis_pembayaran'] == 'transfer' && $data['transfer']): ?>
                    ...
                    <a href="<?= BASE_URL ?>uploads/transfer/<?= $data['transfer'] ?>" target="_blank" class="block w-full">
                        <img src="<?= BASE_URL ?>uploads/transfer/<?= $data['transfer'] ?>" class="w-full h-32 object-cover rounded border hover:opacity-80 transition">
                    </a>
                ...
                <form action="<?= BASE_URL ?>actions/transaksi_update.php" method="POST">
                    ...
                    <?php else: ?>
                        <?php if ($data['status'] == 'Kirim'): ?>
                            ...
                            <button type="submit" name="action" value="selesai" onclick="return confirm('Yakin barang sudah sesuai? Transaksi akan diselesaikan.')"
