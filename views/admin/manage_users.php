                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-gray-200 overflow-hidden">
                                        ...
                                        <?php else: ?>
                                            <div class="flex items-center justify-center h-full font-bold text-gray-500"><?= substr($row['username'], 0, 1) ?></div>
                                        <?php endif; ?>
                                    </div>
                                    <div>
                            ...
                            <td class="p-4">
                                <span class="px-2 py-1 text-xs font-bold rounded <?= $row['role'] == 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700' ?>">
                                    <?= strtoupper($row['role']) ?>
                                </span>
                            </td>
                            <td class="p-4">
                            ...
                            <td class="p-4 flex justify-center gap-2">
                                <form action="<?= BASE_URL ?>actions/admin_users.php" method="POST" onsubmit="return confirm('Ubah role user ini?');">
                                    <input type="hidden" name="user_id" value="<?= $row['id'] ?>">
                                    <input type="hidden" name="current_role" value="<?= $row['role'] ?>">
                                    <button type="submit" name="toggle_role" class="bg-yellow-400 text-white p-2 rounded hover:bg-yellow-500" title="Ubah Role">
                                        👑
                                    </button>
                                </form>
